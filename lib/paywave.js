...
export function parsePaywaveResult(apiRes) {
  ...
  // 1. Prefer an explicit ResultCode (Safaricom STK standard)
  let code;
  ...
  if (code !== undefined) {
    ...
    isSuccess = codeNum === 0;
    // Only definitive Safaricom failure codes — not "still processing"
    const FAILURE_CODES = [1, 2001, 1032, 1037, 1025, 1019];
    isFailed  = FAILURE_CODES.includes(codeNum);
    return { isSuccess, isFailed };
  }

  // 2. Paywave-native success fields
  const successField = apiRes.success;
  if (successField === true || successField === 'true' ||
      successField === '200' || successField === 200) {
    // But only if there's no ResultCode pointing to failure above
    isSuccess = true;
    return { isSuccess, isFailed };
  }

  // 3. String status field
  const s = (apiRes.status || apiRes.Status || apiRes.TransactionStatus || '')
  ...
  // Note: do NOT mark isFailed for generic "fail" keyword — 
  // Paywave might return "failed to find transaction" for pending ones.
