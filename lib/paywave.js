...
export async function initiatePaywaveDirectStkPush({ email, amount, phoneNumber, reference }) {
  ...
  console.log(`[PAY] PayWave STK Push request: using email="${emailToSend}" (isMerchantEmailConfigured=${!!PAYWAVE_MERCHANT_EMAIL}), apiKeyPrefix="${apiKey ? apiKey.substring(0, 6) : 'missing'}..."`);
